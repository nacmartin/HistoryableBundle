...
class NacmartinHistoryableExtension extends Extension
{
    ...
    public function load(array $configs, ContainerBuilder $container)
    {
        ...
        foreach ($config['orm'] as $name => $listeners) {
            foreach ($listeners as $ext => $enabled) {
                $listener = sprintf('nacmartin_historyable.listener.%s', $ext);
        ...
        foreach ($config['class'] as $listener => $class) {
            $container->setParameter(sprintf('nacmartin_historyable.listener.%s.class', $listener), $class);
        }
    }
